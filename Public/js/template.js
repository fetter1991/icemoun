!function () {
    ...
    function r(r, t) {
        ...
        var o = t.debug, c = t.openTag, l = t.closeTag, s = t.parser, u = t.compress, f = t.escape, p = 1, g = {
            $data: 1,
            $filename: 1,
            $utils: 1,
            $helpers: 1,
    ...
    a.parser = function (e, n) {
        e = e.replace(/^\s/, "");
        var r = e.split(" "), a = r.shift(), i = r.join(" ");
        switch (a) {
            case"if":
                e = "if(" + i + "){";
                break;
            case"else":
                r = "if" === r.shift() ? " if(" + r.join(" ") + ")" : "", e = "}else" + r + "{";
                break;
            case"/if":
                e = "}";
                break;
            case"each":
                var o = r[0] || "$data", c = r[1] || "as", l = r[2] || "$value", s = r[3] || "$index", u = l + "," + s;
            ...
            default:
                if (/^\s*\|\s*[\w\$]/.test(i)) {
                    ...
                    for (var p = 0, $ = e.split("|"), d = $.length, g = $[p++]; d > p; p++)g = k(g, $[p]);
                    e = (f ? "=" : "=#") + g
                } else e = t.helpers[a] ? "=#" + a + "(" + r.join(",") + ");" : "=" + e
        }
        return e
